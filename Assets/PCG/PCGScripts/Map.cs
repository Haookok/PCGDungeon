	...
	public class Map : MonoBehaviour
	{
		...
		public IEnumerator Generate()
		{
...

			{
				...
				//yield return PillarCheck();

				//yield return GeneratePillars();
		...
		/*private IEnumerator PillarCheck()
		{
			for (int x = 0; x < MapSize.x; x++)
			{
				for (int z = 0; z < MapSize.z; z++)
				{
					if(_tilesTypes[x, z] == TileType.Empty && IsCornerPosition(x, z))
					{
						_tilesTypes[x, z] = TileType.Pillar;
					}
				}
			}
		...
		{
			if (_tilesTypes[x, z] != TileType.Empty)
				return false;
			
			bool north = IsWallOrBoundary(x, z + 1);
			bool south = IsWallOrBoundary(x, z - 1);
			bool east = IsWallOrBoundary(x + 1, z);
			bool west = IsWallOrBoundary(x - 1, z);
			
			return (north && east) || (north && west) || (south && east) || (south && west);
		}*/

		private bool IsWallOrBoundary(int x, int z)
		{
			if (x < 0 || x >= MapSize.x || z < 0 || z >= MapSize.z)
				return true;
			return _tilesTypes[x, z] == TileType.Wall;
		}
		...
		/*private IEnumerator GeneratePillars()
		{
			...
			pillarsParent.transform.parent = transform;
			
			for(int x = 0; x < MapSize.x; x++)
			{
				for(int z = 0; z < MapSize.z; z++)
				{
					if(_tilesTypes[x, z] == TileType.Pillar)
					{
						CreatePillar(new IntVector2(x, z), pillarsParent.transform);
					}
				}
			}
		...
		private void CreatePillar(IntVector2 coordinates, Transform parent)
		{
			if (PillarPrefab == null)
			{
				Debug.LogWarning("PillarPrefab is not set!");
				return;
			}

			GameObject newPillar = Instantiate(PillarPrefab);
			newPillar.name = "Pillar (" + coordinates.x + ", " + coordinates.z + ")";
			newPillar.transform.parent = parent;
			
			Vector3 worldPosition = CoordinatesToPosition(coordinates) * RoomMapManager.TileSize;
			newPillar.transform.localPosition = worldPosition;
