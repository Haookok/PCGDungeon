...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        //自己添加一个动画ID，实现鼠标左键按下放出动画
        private int _animIDAttack;
        private int _animIDAttack2;

        private int _animIDAttackSpecial;
        //是否在攻击
        ...
        private void AssignAnimationIDs()
        {
            ...
            _animIDAttackSpecial = Animator.StringToHash("AttackSpecial");
        }
        ...
        private void Attack()
        {
            ...
            if (_hasAnimator && Input.GetMouseButtonDown(0) && !_isAttacking && Grounded)
            {
                ...
                _animator.SetFloat(_animIDSpeed, 0f); // 停止移动动画
                _animator.SetFloat(_animIDMotionSpeed, 0f); // 停止移动动画
                
                _animator.SetTrigger(_animIDAttack); // 触发攻击动画
            ...
            if(_hasAnimator && Input.GetMouseButtonDown(1) && !_isAttacking && Grounded)
            {
                ...
                _animator.SetFloat(_animIDSpeed, 0f); // 停止移动动画
                _animator.SetFloat(_animIDMotionSpeed, 0f); // 停止移动动画
                _animator.SetTrigger(_animIDAttack2); // 触发第二个攻击动画
            ...
            {
                Debug.Log("E按下");
                _isAttacking = true; // 设置攻击状态为真，防止重复触发
                _input.jump = false;
                
                _animator.SetFloat(_animIDSpeed, 0f); 
                _animator.SetFloat(_animIDMotionSpeed, 0f); 
                
                _animator.SetTrigger(_animIDAttackSpecial); // 触发大招动画
                Debug.Log("大招触发");
        ...
        {
            yield return new WaitForSeconds(0.79f); // 等待攻击动画完成，假设动画时长为1秒
            _input.jump = false;
            Debug.Log("攻击动画完成，恢复控制");
            _isAttacking = false; // 重置攻击状态为假
        }
        
        private System.Collections.IEnumerator ResetAttackState2()
        {
            yield return new WaitForSeconds(2.1f); // 等待大招动画完成，假设动画时长为1秒
            _input.jump = false;
            Debug.Log("大招动画完成，恢复控制");
            _isAttacking = false; // 重置攻击状态为假
        }
